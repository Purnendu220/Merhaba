...
import com.wpits.merhaba.adapter.viewholder.SongsViewHolder;
...
public class AdapterGridView extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdapterGridView(Context context, int shownInScreen, boolean showLoader, AdapterCallbacks<Object> adapterCallbacks) {
    ...
    public void addClass(Category model) {
        list.add(model);
        addLoader();
    }

    public void addAllClass(List<Category> models) {
        list.addAll(models);
        addLoader();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CLASS) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.view_grid_item_new, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof GridItemViewHolder) {
            if (getItem(position) != null)
                ((GridItemViewHolder) holder).bind(getItem(position), adapterCallbacks, position);
